...
def test_direct_normalization():
    ...
    assert normalized1["sent"] == "false", f"Expected 'false', got {normalized1['sent']}"
    assert isinstance(normalized1["sent"], str), f"Expected str, got {type(normalized1['sent'])}"
    ...
    assert isinstance(normalized2["sent"], str), f"Expected str, got {type(normalized2['sent'])}"
...
def test_webhook_with_delivery_error():
    ...
    with patch('app.clients.evolution_api.EvolutionAPIClient.send_text_message') as mock_send:
        ...
        with patch('app.core.langgraph_flow.run') as mock_run:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "sent" in data:
                        assert isinstance(data["sent"], str), f"sent must be string, got {type(data['sent'])}"
                        assert data["sent"] in ["true", "false"], f"sent must be 'true' or 'false', got {data['sent']}"
                        print(f"✅ sent field is correctly typed as string: '{data['sent']}'")
                    else:
