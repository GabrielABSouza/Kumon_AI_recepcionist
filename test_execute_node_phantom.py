            ...
            with patch("app.core.langgraph_flow.save_conversation_state") as mock_save:
                with patch("app.core.langgraph_flow.get_openai_client") as mock_openai:
                    with patch("app.core.langgraph_flow.send_text") as mock_send:
                        ...
                        assert (
                            mock_save.called
                        ), "save_conversation_state deve ter sido chamado"

                        # Inspecionar TODAS as chamadas para save_conversation_state
                        print(
                            f"FORENSIC: Total de chamadas save_conversation_state: {mock_save.call_count}"
                        )
                        ...
                        for i, call in enumerate(mock_save.call_args_list):
                            ...
                            print(
                                f"FORENSIC: Chamada {i+1}: Telefone={saved_phone}, Estado={saved_state_dict}"
                            )
                        ...
                        print(
                            f"   _execute_node perpetuou: parent_name={phantom_parent_name}"
                        )
                ...
                with patch("app.core.langgraph_flow.get_openai_client") as mock_openai:
                    with patch("app.core.langgraph_flow.send_text") as mock_send:
                        ...
                        if mock_save.called:
                            ...
                            assert (
                                "parent_name" not in saved_state
                                or saved_state.get("parent_name") == ""
                            ), "greeting_node NÃO deve salvar/perpetuar parent_name"

                            print(
                                "✅ CORREÇÃO VALIDADA: _execute_node não perpetua contaminação"
                            )
