    ...
    def test_information_node_blends_answer_with_next_qualification_question(self):
        ...
        with patch("app.core.langgraph_flow.get_openai_client") as mock_openai:
            with patch("app.core.langgraph_flow.send_text") as mock_send_text:
                ...
                assert result.get("sent") in ["true", "false"], (
                ...
                information_keywords = ["horários", "funcionamento", "14h", "20h", "segunda", "sexta"]
                information_present = any(keyword in response_text.lower() for keyword in information_keywords)
                ...
                qualification_present = any(keyword in response_text.lower() for keyword in qualification_keywords)
    ...
    def test_information_node_skips_qualification_when_complete(self):
        ...
        with patch("app.core.langgraph_flow.get_openai_client") as mock_openai:
            with patch("app.core.langgraph_flow.send_text") as mock_send_text:
                ...
                method_keywords = ["método", "kumon", "autodidatismo", "raciocínio", "individualizada"]
                method_present = any(keyword in response_text.lower() for keyword in method_keywords)
                assert method_present, (
                ...
                qualification_questions = ["qual", "nome", "idade", "interesse", "você gostaria"]
                qualification_asked = any(question in response_text.lower() for question in qualification_questions)
                ...
                print(f"📊 Response: {response_text}")
                print("✅ SUCCESS: Pure information response when qualification complete")
    ...
    def test_information_node_adapts_qualification_question_to_missing_data(self):
        ...
        with patch("app.core.langgraph_flow.get_openai_client") as mock_openai:
            with patch("app.core.langgraph_flow.send_text") as mock_send_text:
                ...
                assert "nome" in response1.lower() and "criança" in response1.lower(), (
                    f"Should ask for student name when missing. Response: {response1}"
                )
                
                # Second response should ask for age
                assert "idade" in response2.lower() and "pedro" in response2.lower(), (
                    f"Should ask for age when student name known. Response: {response2}"
                )
                
                print("✅ SUCCESS: Adaptive qualification questions based on missing data")
    ...
    def test_information_node_handles_complex_information_requests(self):
        ...
        with patch("app.core.langgraph_flow.get_openai_client") as mock_openai:
            with patch("app.core.langgraph_flow.send_text") as mock_send_text:
                ...
                addresses_prices = "valor" in response_text.lower() or "r$" in response_text.lower()
                addresses_transport = "transporte" in response_text.lower()
                includes_qualification = "nome" in response_text.lower() and "criança" in response_text.lower()
                
                assert addresses_schedules, f"Should address schedules. Response: {response_text}"
                assert addresses_prices, f"Should address prices. Response: {response_text}"  
                assert addresses_transport, f"Should address transport. Response: {response_text}"
                assert includes_qualification, f"Should include qualification question. Response: {response_text}"
                
                print("✅ SUCCESS: Complex information requests handled with qualification blend")
