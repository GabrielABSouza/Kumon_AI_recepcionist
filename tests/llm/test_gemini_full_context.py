...
class TestGeminiFullContext:
    ...
    def test_classifier_builds_prompt_with_full_context(self):
        ...
        try:
            ...
            assert "HISTÓRICO DA CONVERSA" in prompt or "CONVERSATION HISTORY" in prompt, \
            ...
            assert "meu nome é Gabriel" in prompt, \
            ...
            assert "greeting_sent" in prompt.lower() or "saudação" in prompt.lower(), \
            ...
            pytest.fail(f"EXPECTED FAILURE: _build_contextual_prompt method not implemented yet: {e}")
    ...
    def test_classifier_formats_conversation_history_correctly(self):
        ...
        try:
            ...
            assert "Assistente:" in prompt or "Assistant:" in prompt, \
                "History should be formatted with role labels"
            
            assert "Usuário:" in prompt or "User:" in prompt, \
                "History should include user role labels"
            
            # Should include actual conversation content
            assert "Quero informações sobre Kumon" in prompt, \
                "Should include user messages from history"
            
            print("✅ Conversation history formatting validated")
            
        except AttributeError:
            pytest.fail("EXPECTED FAILURE: _build_contextual_prompt method not implemented yet")
    ...
    def test_classifier_includes_state_variables_in_prompt(self):
        ...
        try:
            ...
            assert "mathematics" in prompt, "Should include program_interests from state"
            
            # Should indicate what's missing
            assert "student_age" in prompt.lower() or "idade" in prompt.lower(), \
    ...
    def test_classifier_handles_empty_context_gracefully(self):
        ...
        try:
            ...
            assert "olá" in prompt, "Should include user message even with empty context"
    ...
    def test_classifier_classify_method_accepts_context_parameter(self):
        ...
        with patch.dict('os.environ', {'GEMINI_API_KEY': 'test-key'}), \
            ...
            try:
                ...
                assert isinstance(intent, Intent), f"Expected Intent enum, got {type(intent)}"
                assert isinstance(confidence, float), f"Expected float confidence, got {type(confidence)}"
                assert intent == Intent.QUALIFICATION, f"Expected QUALIFICATION, got {intent}"
                assert confidence == 0.9, f"Expected 0.9 confidence, got {confidence}"
                
                print("✅ Context parameter accepted by classify method")
                
            except TypeError as e:
                pytest.fail(f"EXPECTED FAILURE: classify method doesn't accept context parameter yet: {e}")
