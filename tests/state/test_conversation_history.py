...
class TestConversationHistory:
    ...
    def test_get_conversation_history_returns_recent_messages(self):
        ...
        with patch('app.core.state_manager._get_stored_messages') as mock_get_messages:
            ...
            assert len(result) == 4, \
                f"Expected 4 messages (limit=4), got {len(result)}"
            ...
            expected_last_content = "Perfeito! Qual é a idade do João?"  # This is the most recent message
            actual_last_content = result[-1]["content"]
            assert actual_last_content == expected_last_content, \
            ...
            expected_first_content = "meu nome é Gabriel"  # 2nd message in mock becomes 1st in result
            actual_first_content = result[0]["content"]
            assert actual_first_content == expected_first_content, \
            ...
            assert result[0]["role"] == "user", "First message in result should be user response"
    ...
    def test_get_conversation_history_respects_limit_parameter(self):
        ...
        with patch('app.core.state_manager._get_stored_messages') as mock_get_messages:
            ...
            mock_get_messages.return_value = [
                {"timestamp": f"2023-01-01T10:0{i}:00Z", "role": "user" if i % 2 else "assistant", 
                 "content": f"Message {i}"}
    ...
    def test_get_conversation_history_default_limit(self):
        ...
        with patch('app.core.state_manager._get_stored_messages') as mock_get_messages:
            ...
            assert len(result) == 4, \
                f"Expected default limit of 4, got {len(result)}"
