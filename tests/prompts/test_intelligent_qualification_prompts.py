...
class TestIntelligentQualificationPrompts:
    ...
    def test_prompts_follow_required_vars_sequence(self):
        ...
        assert "parent_name" == QUALIFICATION_REQUIRED_VARS[0], "Test expects parent_name to be first"
        assert "nome" in result["system"].lower(), f"Should ask for name, got: {result['system']}"
    ...
    def test_prompts_ask_for_next_missing_variable(self):
        ...
        assert "idade" in result["system"].lower(), (
        ...
        assert any(word in system_lower for word in ["disciplina", "matemática", "português"]), (
            f"Should ask for program interests, got: {result['system']}"
        )
        assert "Ana" in result["system"] and "10 anos" in result["system"], (
    ...
    def test_complete_state_offers_next_steps(self):
        ...
        system_lower = result["system"].lower()
        assert "perfeito" in system_lower or "ótimo" in system_lower, (
    ...
    def test_beneficiary_type_handling(self):
        ...
        assert any(phrase in system_lower for phrase in [
            "para você", "para outra pessoa", "beneficiário"
        ...
        result = get_qualification_prompt("Para minha filha", child_beneficiary, attempts=1)
        
        # Should ask for child's name specifically
        system_lower = result["system"].lower()
        assert any(word in system_lower for word in ["nome", "criança", "filha"]), (
    ...
    def test_escape_hatch_after_multiple_attempts(self):
        ...
        assert any(word in system_lower for word in ["telefone", "informações"]), (
    ...
    def test_missing_vars_identification(self):
        ...
        test_cases = [
            ...
            ({"parent_name": "Test"}, 3),  # Parent name IS in QUALIFICATION_REQUIRED_VARS - 1 present, 3 missing
            ({"parent_name": "Test", "student_name": "Test"}, 2),  # 2 present, 2 missing  
            ({"parent_name": "Test", "student_name": "Test", "student_age": 10}, 1),  # 3 present, 1 missing
            ({"parent_name": "Test", "student_name": "Test", "student_age": 10, "program_interests": ["math"]}, 0),  # All present
        ]
        ...
        for state, expected_missing_count in test_cases:
            ...
            missing_count = sum(
                1 for var in QUALIFICATION_REQUIRED_VARS 
    ...
    def test_enhanced_personalization(self):
        ...
        result = get_qualification_prompt("Ela tem 12 anos", personalized_state, attempts=1)
