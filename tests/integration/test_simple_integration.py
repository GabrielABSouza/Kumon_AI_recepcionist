...
async def test_integration_with_direct_mocks():
    ...
    with patch('app.core.routing.master_router.classifier') as mock_classifier, \
         patch('app.core.gemini_classifier.classifier') as mock_classifier2, \
         patch('app.core.state_manager.get_conversation_state') as mock_get_state, \
         patch('app.core.state_manager.save_conversation_state') as mock_save_state, \
         patch('app.core.state_manager.get_conversation_history') as mock_get_history, \
         patch('app.core.delivery.send_text') as mock_send_text, \
         patch('app.core.llm.openai_adapter.OpenAIClient.chat') as mock_llm_chat:
    ...
    assert mock_llm_chat.called, "LLM deveria ter sido chamado para gerar resposta blended"
