...
async def test_complete_specialized_prompts_flow_integration():
    ...
    with patch("app.core.delivery.send_text", new_callable=AsyncMock) as mock_send_text:
        with patch(
            ...
            assert (
                "pending_input_for" in turno1_result
            ...
            print(
                f"🔍 TURNO 1 - pending_input_for: {turno1_result.get('pending_input_for')}"
            )
            ...
            if mock_send_text.called:
                ...
                assert (
                    "Cecília" in call_args["text"]
                ), "Mensagem deve conter nome da assistente"
    ...
    with patch("app.core.state_manager.get_conversation_state") as mock_get_state:
        with patch(
            ...
            with patch(
                "app.core.delivery.send_text", new_callable=AsyncMock
            ) as mock_send_text:
                with patch(
                    "app.core.langgraph_flow.gemini_classifier.classify",
                    ...
                    mock_get_history.return_value = [
                        {
                            "role": "assistant",
                            "content": "Olá! Eu sou a Cecília do Kumon Vila A. Qual é o seu nome?",
                        }
                    ]
                    mock_send_text.return_value = {"sent": "true", "status_code": 200}
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        assert (
                            "task" in kwargs
                        ), f"Caso {i+1}: task deveria estar presente"
                        ...
                        print(
                            f"✅ CASO {i+1}: Task '{case['expected_task']}' passada corretamente"
                        )
