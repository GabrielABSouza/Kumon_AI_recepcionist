...
def test_qualification_node_extracts_parent_name_from_message_when_missing():
    ...
    with patch(
        ...
        saved_state_call = mock_save_state.call_args[0][
            1
        ]  # Get the state that was saved
        saved_phone_call = mock_save_state.call_args[0][0]  # Get the phone number used

        print(
            f"DEBUG: save_conversation_state called with phone='{saved_phone_call}', state={saved_state_call}"
        )

        assert (
            "parent_name" in saved_state_call
        ), "Should have extracted and saved parent_name"
        assert (
            saved_state_call["parent_name"] == "Gabriel"
        ...
        assert any(
            word in response
            for word in ["quem", "beneficiário", "matrícula", "para quem"]
        ), f"Should ask next qualification question, got: {response}"

        print("✅ TDD STEP 2.1: parent_name extraction test structure created")
        print(
            f"   Validated: Local extraction in qualification_node when parent_name missing"
        )
...
def test_qualification_node_does_not_extract_when_parent_name_already_exists():
    ...
    with patch(
        "app.core.langgraph_flow.get_conversation_state"
    ) as mock_get_state, patch("app.core.langgraph_flow.send_text") as mock_send, patch(
        "app.core.langgraph_flow.save_conversation_state"
    ) as mock_save_state:
        ...
        if mock_save_state.called:
            ...
            assert (
                saved_state_call["parent_name"] == "Maria"
