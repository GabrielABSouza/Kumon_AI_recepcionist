...
def test_end_to_end_qualification_flow_with_local_extraction():
    ...
    with patch("app.core.langgraph_flow.get_conversation_state") as mock_get_state, \
        ...
        assert mock_save_state.called, "Should have saved conversation state after extraction"
        ...
        assert saved_phone == "+5511999999999", f"Should save for correct phone, got: {saved_phone}"
        assert "parent_name" in saved_state, f"Should have extracted parent_name, saved_state: {saved_state}"
        assert saved_state["parent_name"] == "Ana", f"Should extract 'Ana' from 'Meu nome é Ana', got: {saved_state['parent_name']}"
        
        # ASSERTION 2: Response should be successful
        assert result.get("sent") == "true", f"Should send response successfully, got: {result}"
        
        # ASSERTION 3: System should ask next qualification question  
        response = result.get("response", "").lower()
        assert any(word in response for word in ["nome da criança", "filho", "filha", "criança"]), \
            f"Should ask about child's name next, got response: '{response}'"
            
        print("✅ INTEGRATION TEST PASSED")
        print(f"   ✓ Local extraction working: parent_name='Ana' extracted from 'Meu nome é Ana'")
        ...
        print(f"   ✓ No global extraction: Only triggered locally in qualification_node")
...
def test_integration_no_extraction_when_parent_name_exists():
    ...
    with patch("app.core.langgraph_flow.get_conversation_state") as mock_get_state, \
        ...
        result = qualification_node(state_with_existing_name)
        ...
        if mock_save_state.called:
            ...
            if "parent_name" in saved_state:
                assert saved_state["parent_name"] == "Maria", \
