...
async def reset_all_conversations() -> Dict[str, Any]:
    """Reset all conversation states - useful for testing"""
    try:
        count = len(conversation_flow_manager.conversation_states)
        conversation_flow_manager.conversation_states.clear()
        app_logger.info(f"Reset all {count} conversations")
        return {"message": f"All {count} conversations reset", "conversations_reset": count}
    except Exception as e:
        app_logger.error(f"Error resetting all conversations: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to reset all conversations")
