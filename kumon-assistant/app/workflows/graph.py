...
from .nodes import greeting_node, information_node, scheduling_node, fallback_node
from .edges import (
    route_from_greeting,
...
class KumonWorkflow:
    ...
    def __init__(self):
        ...
        self.config = {
            "configurable": {
                "thread_id": "default"  # Will be overridden per conversation
            }
        }
...
Nossa equipe terá todo prazer em ajudá-lo! 😊"""
    ...
    async def _completion_node(self, state: ConversationState) -> ConversationState:
        ...
        app_logger.info(f"Conversation metrics for {state['phone_number']}: "
    ...
    async def process_message(
        ...
        try:
            ...
            app_logger.info(f"Message processed for {phone_number}. "
            ...
            return {
                ...
                "state": state if 'state' in locals() else create_initial_state(phone_number, user_message),
                "requires_human": True,
                "conversation_ended": True,
