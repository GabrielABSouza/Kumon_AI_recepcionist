...
class IntelligentFallbackSystem:
        ...
        return [
            {
                ...
                "condition": lambda state: any(
                    phrase in state["user_message"].lower()
            ...
            },
            {
                ...
                "condition": lambda state: (
                    state["user_context"].programs_interested
                    and len(state["messages"]) > 5
                    and state["metrics"].consecutive_confusion >= 2
                ),
                "confidence": 0.8,
                "urgency": "high",
            },
            ...
            },
            {
                ...
                "condition": lambda state: (
                    datetime.now() - state["metrics"].start_time > timedelta(minutes=15)
                    and state["metrics"].consecutive_confusion >= 1
                ),
                "confidence": 0.7,
                "urgency": "normal",
            },
    ...
    async def analyze_confusion(self, conversation_state: ConversationState) -> ConfusionAnalysis:
        """
        ...
        try:
            ...
            app_logger.info(
                f"Confusion classified: {best_match.confusion_type.value} "
    ...
    def _extract_context_factors(
        self, confusion_type: ConfusionType, conversation_state: ConversationState
    ) -> Dict[str, Any]:
        ...
        factors["conversation_duration"] = (
            datetime.now() - metrics.start_time
        ).total_seconds() / 60

        # Type-specific factors
        if confusion_type == ConfusionType.TECHNICAL:
            factors["contains_error_keywords"] = any(
                word in user_message.lower() for word in ["erro", "bug", "problema"]
            )
            ...
            factors["explanation_requests"] = len(
                re.findall(r"explain|explique|como", user_message.lower())
            )

        elif confusion_type == ConfusionType.REFERENTIAL:
            factors["pronouns_count"] = len(
                re.findall(r"\b(isso|isto|aquilo|ele|ela)\b", user_message.lower())
            )
            factors["context_gap"] = (
                len(conversation_state["messages"]) - metrics.clarification_attempts
            )
    ...
    async def determine_recovery_action(
        self, confusion_analysis: ConfusionAnalysis, conversation_state: ConversationState
    ) -> RecoveryAction:
        """
        ...
        try:
            ...
            if confusion_analysis.severity > 0.7 or confusion_analysis.is_recurring:
                strategy = strategy_config.get(
                    "fallback_strategy", strategy_config["primary_strategy"]
                )
            else:
            ...
            response = await self._generate_recovery_response(
                strategy, confusion_analysis, conversation_state, strategy_config
            )
            ...
            recovery_action = RecoveryAction(
                ...
                requires_context_reset=strategy
                in [RecoveryStrategy.RESTART, RecoveryStrategy.REDIRECT],
            )

            app_logger.info(
                f"Recovery action determined: {strategy.value} " f"(confidence: {confidence:.2f})"
            )
                ...
                try:
                    response = await prompt_manager.get_prompt(
                        ...
                        variables={
                            ...
                            "user_name": conversation_state["user_context"].parent_name
                            or "cliente",
                        },
                    )
                    if response:
...
Sobre qual dessas opções você gostaria de saber mais?"""
                ...
                return await prompt_manager.get_prompt(
                    "kumon:fallback:handoff:explicit_request", fallback_enabled=True
                )
    ...
    def _determine_recovery_next_step(
        self, strategy: RecoveryStrategy, conversation_state: ConversationState
    ) -> ConversationStep:
    ...
    def _calculate_recovery_confidence(
        ...
        if strategy == RecoveryStrategy.ESCALATE and confusion_analysis.severity > 0.7:
            ...
            strategy == RecoveryStrategy.EXPLANATION
            and confusion_analysis.confusion_type == ConfusionType.CONCEPTUAL
        ):
            base_confidence += 0.15
        elif (
            strategy == RecoveryStrategy.PROVIDE_OPTIONS
            and conversation_state["metrics"].clarification_attempts <= 1
        ):
            base_confidence += 0.1
    ...
    async def should_escalate_to_human(
        ...
        try:
                ...
                try:
                    if rule["condition"](conversation_state):
                        ...
                        reasoning_parts.append(
                            f"{rule['trigger'].value} (confidence: {rule['confidence']})"
                        )
                except Exception as e:
            ...
            if confusion_analysis:
                if (
                    confusion_analysis.confusion_type
                    in [ConfusionType.TECHNICAL, ConfusionType.LINGUISTIC]
                    and confusion_analysis.severity > 0.7
                ):
            ...
            final_confidence = (
                min(0.95, total_confidence / max(1, len(triggers))) if triggers else 0.0
            )

            # Determine urgency
            urgency = "normal"
            if any(
                trigger in [EscalationTrigger.TECHNICAL_ISSUE, EscalationTrigger.DISSATISFACTION]
            ...
            if should_escalate:
                ...
                app_logger.info(
                    f"Escalation decision: {should_escalate} "
    ...
    def get_fallback_stats(self) -> Dict[str, Any]:
        ...
        recovery_rate = self.fallback_stats["successful_recoveries"] / max(
            1, self.fallback_stats["total_confusions"]
        )

        escalation_rate = self.fallback_stats["escalations"] / max(
            1, self.fallback_stats["total_confusions"]
        )
