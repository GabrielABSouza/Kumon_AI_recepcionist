...
class PricingValidationNode:
    ...
    async def execute(
        ...
        try:
            ...
            pricing_keywords = [
                "preço", "valor", "custa", "quanto", "mensalidade",
                "taxa", "matrícula", "pagamento", "dinheiro"
            ]
...
class SchedulingConstraintNode:
    ...
    async def execute(
        ...
        try:
            ...
            scheduling_keywords = [
                "horário", "agenda", "marcar", "agendar", "disponível",
                "manhã", "tarde", "segunda", "terça", "quarta", "quinta", "sexta",
                "sábado", "domingo", "fim de semana"
            ]
            
            contains_scheduling = any(keyword in user_message.lower() for keyword in scheduling_keywords)
...
class LeadQualificationTrackingNode:
    ...
    async def execute(
        ...
        try:
            app_logger.info(f"Executing lead qualification tracking node for {state.get('phone_number')}")
            ...
            if is_qualified:
                ...
                action_required = "continue_qualification"
                enforcement_message = f"Estamos progredindo bem! Ainda preciso de: {', '.join(missing_fields[:3])}"
            else:
                action_required = "request_basic_info"
                enforcement_message = "Para agendar sua avaliação, preciso de algumas informações básicas."
            
            return BusinessRuleNodeResult(
                rule_type=RuleType.QUALIFICATION,
                validation_result=qualification_validation.result,
                compliance_passed=completion_percentage >= 50.0,  # Minimum threshold for progression
...
class BusinessComplianceValidationNode:
    ...
    async def execute(
        ...
        try:
            app_logger.info(f"Executing comprehensive business compliance validation for {state.get('phone_number')}")
            ...
            compliance_percentage = (total_compliance_score / total_rules_evaluated * 100) if total_rules_evaluated > 0 else 100.0
