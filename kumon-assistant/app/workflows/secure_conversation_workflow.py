...
from ..workflows.validators import get_validation_agent, ValidationResult
from ..core.state.models import CeciliaState as ConversationState, ConversationStage as WorkflowStage, ConversationStep
...
class SecureConversationWorkflow:
    ...
    async def _get_or_create_conversation_state(
        ...
        session_id = hashlib.md5(f"{phone_number}_{datetime.now()}".encode()).hexdigest()[:12]
    ...
    async def _execute_secure_workflow(
        ...
        try:
            ...
            sanitized_message = security_result.get("sanitized_message", conversation_state["user_message"])
            ...
            if isinstance(context_result, dict):
                conversation_state["resolved_references"] = context_result.get("resolved_references", [])
            ...
            for rule_type, rule_result in business_rule_results.items():
                if isinstance(rule_result, BusinessRuleNodeResult):
                    ...
                    if not rule_result.compliance_passed:
                        ...
                        app_logger.warning(f"Business rule violation: {rule_type.value} - {rule_result.enforcement_message}")
            ...
            routing_decision = SimpleNamespace(
                target_stage=conversation_state.get("stage", conversation_state.get("current_stage")),
                confidence=0.9
            )
    ...
    async def _resolve_context_references(self, conversation_state: ConversationState) -> Dict[str, Any]:
    ...
    async def _generate_rag_context(self, user_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            rag_result = await langchain_rag_service.query(
                user_message, include_sources=True
            )
            
            # Validate RAG response against business rules
            corrected_content, corrections_made = await rag_business_validator.get_corrected_rag_response(
            ...
            return {
                ...
                "business_validated": len(corrections_made) == 0 or corrections_made[-1] == "No corrections needed"
            }
        except Exception as e:
...
Estou à disposição para esclarecer todas as suas dúvidas!"""
    ...
    async def _rag_business_node(self, state: ConversationState) -> ConversationState:
        ...
        try:
            ...
            if rag_context:
                corrected_content, corrections = await rag_business_validator.get_corrected_rag_response(
                    rag_context,
                    {"user_query": state.get("user_message", ""), "state": state}
                )
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_ratios": {
                "block_rate": (
                    self.security_metrics["blocked_requests"] / max(1, total_requests)
                ),
                "escalation_rate": (
                    self.security_metrics["escalations"] / max(1, total_requests)
                ),
...
# Global secure workflow instance
