...
class SecurityValidationAgent:
    ...
    async def validate_response(
        ...
        layer_results = {}
        security_result, quality_result, scope_result, info_result, business_result = (
            validation_results
        )
        ...
        await self._track_validation_result(source_identifier, validation_report, response)
    ...
    async def _security_validation_layer(
        self, response: str, source_identifier: str, metadata: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def _quality_validation_layer(
        self, response: str, agent_state: AgentState
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        """Você é Cecília, a validadora de qualidade para respostas da assistente do Kumon Vila A.
            ...
            passed = (
                ...
                and validation_data.get("quality_score", 0.0)
    ...
    async def _information_safety_layer(
        self, response: str, metadata: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def _track_validation_result(
        self, source_identifier: str, validation_report: ValidationReport, response: str
    ):
        ...
        if len(self.validation_history[source_identifier]) > 100:
            self.validation_history[source_identifier] = self.validation_history[source_identifier][
                -100:
            ]
...
class QualityMetrics:
    ...
    def update_quality_metrics(
        self, state: AgentState, validation_report: ValidationReport
    ) -> AgentState:
