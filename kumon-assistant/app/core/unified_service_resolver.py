...
class UnifiedServiceResolver:
    ...
    async def get_service(self, service_name: str) -> Any:
        ...
        try:
            ...
            if service_name in self.service_cache:
                ...
                self.logger.debug(f"🎯 Service '{service_name}' resolved from cache ({elapsed*1000:.1f}ms)")
            ...
            if service_instance is not None:
                ...
                self.logger.info(f"✅ Service '{service_name}' resolved via Service Factory ({elapsed*1000:.1f}ms)")
            ...
            if service_instance is not None:
                ...
                self.logger.info(f"✅ Service '{service_name}' resolved via Optimized Startup Manager ({elapsed*1000:.1f}ms)")
                return service_instance
            ...
            if service_instance is not None:
                ...
                self.logger.warning(f"⚠️ Service '{service_name}' resolved via lazy fallback ({elapsed*1000:.1f}ms)")
                return service_instance
            ...
            self.logger.error(f"❌ Failed to resolve service '{service_name}' ({elapsed*1000:.1f}ms): {e}")
    ...
    async def _try_service_factory(self, service_name: str) -> Optional[Any]:
        ...
        try:
            ...
            self.logger.warning(f"Unexpected error in Service Factory resolution for '{service_name}': {e}")
            return None
    ...
    async def _try_optimized_startup_manager(self, service_name: str) -> Optional[Any]:
        ...
        try:
            ...
            self.logger.debug(f"Optimized Startup Manager resolution failed for '{service_name}': {e}")
            return None
        except Exception as e:
            self.logger.warning(f"Unexpected error in Optimized Startup Manager resolution for '{service_name}': {e}")
            return None
    ...
    def _update_metrics(self, elapsed_time: float):
        ...
        self.metrics.avg_resolution_time = (
            (self.metrics.avg_resolution_time * (total_successful - 1) + elapsed_time) / total_successful
        )
    ...
    def _generate_detailed_error_message(self, service_name: str, elapsed_time: float) -> str:
        ...
        service_factory_services = list(self.service_factory._service_configs.keys()) if self.service_factory else []
        optimized_services = list(self.optimized_startup_manager.services.keys()) if self.optimized_startup_manager else []
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            health_status = {
                ...
                "available_services": {
                    "service_factory": list(self.service_factory._service_configs.keys()) if self.service_factory else [],
                    "optimized_startup": list(self.optimized_startup_manager.services.keys()) if self.optimized_startup_manager else [],
