...
@router.get("/units/{user_id}/webhook")
async def verify_unit_webhook(
    ...
    if not unit or not unit.config.is_active:
        app_logger.error(f"Unit not found or inactive: {user_id}")
    ...
    app_logger.info(f"Unit webhook verification successful for unit: {user_id}")
    # Return the challenge to verify the webhook
    return PlainTextResponse(content=hub_challenge)


@router.post("/units/{user_id}/webhook")
            ...
            for change in entry.changes:
                ...
                if change.get("field") == "messages":
                    ...
                    for message_data in messages:
                        await process_unit_message(message_data, value, user_id)
        
        return WebhookResponse(status="success")
        
    except Exception as e:
        app_logger.error(f"Error processing unit webhook: {str(e)}", extra={"user_id": user_id})
        raise HTTPException(status_code=500, detail="Error processing webhook")


async def process_unit_message(message_data: Dict[str, Any], value: Dict[str, Any], user_id: str):
    ...
    try:
        ...
        unit = unit_manager.get_unit(user_id)
        if not unit:
            raise Exception(f"Unit not found: {user_id}")
        ...
        if to_number != unit.config.whatsapp_phone_number_id:
            app_logger.warning(f"Message phone number mismatch for unit {user_id}", extra={
        ...
        whatsapp_message = WhatsAppMessage(
            ...
            metadata={
                **message_data,
                "user_id": user_id,
                "unit_context": unit_manager.get_unit_context(user_id)
            }
        )
...
@router.get("/units/{user_id}/status")
async def get_unit_webhook_status(user_id: str = Path(..., description="User ID")):
    ...
    return {
        ...
        "webhook_url": f"{settings.WHATSAPP_WEBHOOK_URL}/units/{user_id}/webhook",
