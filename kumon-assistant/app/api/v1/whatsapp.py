...
app_logger.info("🚀 WhatsApp route using CeciliaWorkflow ONLY (LangGraph-based conversation system)")
...
async def handle_evolution_webhook(request: Request):
    ...
    try:
        ...
        app_logger.info("Evolution API webhook received - Phase 2 Pipeline Integration", extra={
        ...
        app_logger.info("🚀 Processing through Pipeline Orchestrator with enterprise-grade error handling")
        ...
        if pipeline_result.status.value == "completed":
            ...
            response = {
                ...
                "stage_performance": {
                    ...
                    for stage in ["preprocessing", "business_rules", "langgraph_workflow", "postprocessing", "delivery"]
                }
            }
...
async def process_incoming_message(message_data: Dict[str, Any], value: Dict[str, Any]):
    ...
    try:
        ...
        app_logger.info("🚀 Processing message through CeciliaWorkflow (LangGraph) - PRIMARY SYSTEM")
        ...
        try:
            ...
            response = MessageResponse(
                ...
                metadata={
                    ...
                    "coordination_method": workflow_result.get("coordination_method", "direct_langgraph"),
...
async def test_preprocessor():
    ...
    try:
        ...
        # Test headers
        test_headers = {
            "apikey": settings.EVOLUTION_API_KEY or "test-key"
        }
...
async def security_health_check():
    ...
    try:
        ...
        return {
            ...
            "error": str(e),
            "components": {
                "secure_message_processor": "unavailable"
            }
        }
...
async def cecilia_workflow_metrics():
    ...
    try:
        ...
        return {"error": "CeciliaWorkflow metrics unavailable", "timestamp": datetime.now().isoformat()}
    ...
    try:
        ...
        return {
            ...
            "cecilia_workflow_test": {
                ...
                "stage": workflow_result.get("stage"),
                "step": workflow_result.get("step"),
                "performance_assessment": "excellent" if processing_time < 3000 else "good" if processing_time < 5000 else "needs_improvement"
            },
            "system_status": {
                "workflow_system": "cecilia_langgraph",
                "architecture": "langgraph_based",
...
async def test_cache_performance():
    ...
    try:
        ...
        return {
            "timestamp": datetime.now().isoformat(),
            "wave_2_cache_test": test_results
        }
...
async def pipeline_health_report():
    ...
    try:
        ...
        return {
            ...
            "health_report": {
                ...
                "stage_metrics": {
                    stage_name: {
                        ...
                        "success_rate": round((metrics.successful_executions / max(1, metrics.total_executions)) * 100, 2),
            ...
            "sla_target_ms": 3000,
            "performance_status": "excellent" if health_report.overall_health_score > 90 else "good" if health_report.overall_health_score > 75 else "needs_attention"
        }
        
    except Exception as e:
        app_logger.error(f"Pipeline health check failed: {e}")
        return {"error": "Pipeline health check unavailable", "timestamp": datetime.now().isoformat()}
...
async def pipeline_alerts(level: str = None):
    """Get active pipeline alerts"""
    try:
        from ...services.pipeline_monitor import pipeline_monitor
        from ...services.pipeline_monitor import AlertLevel
...
async def pipeline_circuit_breakers():
    ...
    try:
        ...
        return {"error": "Circuit breaker status unavailable", "timestamp": datetime.now().isoformat()}
