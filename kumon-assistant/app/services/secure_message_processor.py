...
from ..models.message import MessageResponse, WhatsAppMessage
...
class SecureMessageProcessor:
    ...
    async def process_message(self, whatsapp_message) -> MessageResponse:
        ...
        phone_number = getattr(whatsapp_message, "phone", None) or getattr(
            whatsapp_message, "from_number", None
        )
        message_content = getattr(whatsapp_message, "message", None) or getattr(
            whatsapp_message, "content", None
        )
        ...
        try:
            app_logger.info(
                f"Processing message {message_id} from {phone_number}", extra=processing_metadata
            )
            ...
            if not security_pre_check["allowed"]:
                ...
                return self._create_secure_response(
                    whatsapp_message, security_pre_check["response"], processing_metadata
                )
            ...
            self.metrics.avg_processing_time = (
                self.metrics.avg_processing_time * (self.metrics.processed_messages - 1)
                + processing_time
            ...
            return self._create_secure_response(
                whatsapp_message, self.fallback_responses["timeout"], processing_metadata
            )

        except Exception as e:
            self.metrics.error_messages += 1
            app_logger.error(
                f"Message processing error for {phone_number}: {e}", extra=processing_metadata
            )

            return self._create_secure_response(
                whatsapp_message, self.fallback_responses["processing_error"], processing_metadata
            )
    ...
    async def _pre_processing_security_validation(
        ...
        try:
            ...
            app_logger.info(
                f"DEBUG: Security action={security_action.value}, context keys={list(security_context.keys())}"
            )
            ...
            if security_score > 0.8:  # High threat score
                app_logger.warning(
                    f"High security score ({security_score}) for {phone_number}", extra=metadata
                )
    ...
    async def _sanitize_and_normalize_input(
        self, message_content: str, metadata: Dict[str, Any]
    ) -> str:
        ...
        try:
            ...
            if len(sanitized_content) > max_length:
                ...
                app_logger.warning(
                    f"Message truncated due to length: {len(message_content)} chars", extra=metadata
                )
    ...
    async def _execute_secure_workflow(
        ...
        try:
            ...
            workflow_result = await asyncio.wait_for(
                workflow_task, timeout=self.config["max_processing_time"]
            )
    ...
    async def _post_processing_validation(
        self, workflow_result, original_message: WhatsAppMessage, metadata: Dict[str, Any]
    ) -> MessageResponse:
        ...
        try:
            ...
            if workflow_result.action_taken == SecureWorkflowAction.BLOCK:
                ...
                if workflow_result.quality_score < 0.5:
                    ...
                    app_logger.warning(
                        f"Low quality response: {workflow_result.quality_score}", extra=metadata
                    )
            ...
            return self._create_secure_response(original_message, response_content, metadata)

        except Exception as e:
            app_logger.error(f"Post-processing validation error: {e}")
            return self._create_secure_response(
                original_message, self.fallback_responses["validation_failure"], metadata
            )

    async def _final_response_security_check(
        self, response_content: str, metadata: Dict[str, Any]
    ) -> Dict[str, bool]:
    ...
    def _create_secure_response(
        self, original_message: WhatsAppMessage, content: str, metadata: Dict[str, Any]
    ) -> MessageResponse:
        ...
        to_number = getattr(original_message, "phone", None) or getattr(
            original_message, "from_number", None
        )
    ...
    def get_processing_metrics(self) -> Dict[str, Any]:
        ...
        success_rate = self.metrics.processed_messages / max(1, self.metrics.total_messages)

        security_incident_rate = self.metrics.security_incidents / max(
            1, self.metrics.total_messages
        )
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "escalation_rate": self.metrics.escalated_messages
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "timestamp": datetime.now().isoformat(),
                "error": str(e),
                "components": {"secure_message_processor": "degraded"},
            }
