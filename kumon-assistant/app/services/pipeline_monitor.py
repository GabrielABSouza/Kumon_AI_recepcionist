...
class BottleneckAnalyzer:
    ...
    async def analyze_bottlenecks(
        self,
        stage_metrics: Dict[str, StageMetrics],
        target_total_time_ms: float = 3000
        ...
        try:
            ...
            total_avg_time = sum(
                metrics.avg_duration_ms for metrics in stage_metrics.values()
...
class PipelineMonitor:
    ...
    def __init__(self):
        ...
        app_logger.info("Pipeline Monitor initialized successfully", extra={
    ...
    async def _update_stage_metrics(
        ...
        duration_ms = stage_result.get("processing_time_ms", 0) or stage_result.get("duration_ms", 0)
        if duration_ms > 0:
    ...
    async def _generate_alert(
        ...
        try:
            ...
            if alert_key in self.active_alerts:
                ...
                if (datetime.now() - last_alert_time).total_seconds() < self.monitoring_config["alert_cooldown_seconds"]:
            ...
            # Log alert
            app_logger.warning(f"Performance Alert [{level.value.upper()}]: {message}", extra={
    ...
    async def _perform_bottleneck_analysis(self):
        ...
        try:
            ...
            app_logger.info(f"Bottleneck analysis completed: {len(bottlenecks)} bottlenecks, {len(recommendations)} recommendations")
    ...
    async def get_pipeline_health_report(self) -> PipelineHealthReport:
        ...
        try:
            ...
            total_executions = sum(metrics.total_executions for metrics in self.stage_metrics.values())
            total_successful = sum(metrics.successful_executions for metrics in self.stage_metrics.values())
            total_avg_time = sum(metrics.avg_duration_ms for metrics in self.stage_metrics.values() if metrics.total_executions > 0)
            ...
            health_score = (
                (success_rate * 0.4) +
                (min(100, (self.monitoring_config["sla_target_ms"] / max(1, total_avg_time)) * 100) * 0.3) +
