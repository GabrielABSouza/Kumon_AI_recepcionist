...
class VectorStore:
    ...
    async def initialize(self) -> None:
        ...
        try:
            # Initialize Qdrant client with reduced timeout for faster fallback
            if settings.QDRANT_API_KEY:
                self.client = QdrantClient(
                    url=settings.QDRANT_URL,
                    api_key=settings.QDRANT_API_KEY,
                    timeout=60,  # Maximum timeout for Railway network
                )
            else:
                self.client = QdrantClient(
                    url=settings.QDRANT_URL,
            ...
            app_logger.warning(
                "🚨 Activating vector store fallback mode - using local knowledge base"
            )
            from ..core.robust_fallbacks import robust_fallback_manager

            robust_fallback_manager.activate_fallback(
                "vector", f"Qdrant initialization failed: {e}"
            )
        ...
        try:
            ...
            for doc in documents:
                if doc.embedding is not None:
                    app_logger.info(
                        f"Creating PointStruct with ID: {doc.id} (type: {type(doc.id)})"
                    )
        ...
        try:
            ...
            if keyword_filter:
                ...
                for keyword in keyword_filter:
                    filter_conditions.append(
                        FieldCondition(key="keywords", match=MatchValue(value=keyword))
                    )
