...
class RecoveryMetrics:
    ...
    stage_recovery_stats: Dict[str, Dict[str, int]] = None
    ...
    async def attempt_recovery(
        ...
        try:
            ...
            if attempt_number <= 2 and "validation" in str(error).lower():
                app_logger.info(f"Preprocessing recovery attempt {attempt_number}: simplified validation")
                ...
                recovery_context = {
                    ...
                    "prepared_context": {
                        ...
                        "last_user_message": message.message if hasattr(message, 'message') else str(message),
            ...
            # Strategy 2: Bypass preprocessing with safe defaults
                ...
                app_logger.warning(f"Preprocessing recovery attempt {attempt_number}: bypassing with defaults")
...
class BusinessRulesRecoveryHandler(StageRecoveryHandler):
    ...
    async def attempt_recovery(
        ...
        try:
            ...
            if attempt_number <= 2:
                app_logger.info(f"Business rules recovery attempt {attempt_number}: relaxed validation")
            ...
            # Strategy 2: Bypass business rules
                ...
                app_logger.warning(f"Business rules recovery attempt {attempt_number}: bypassing validation")
...
class LangGraphRecoveryHandler(StageRecoveryHandler):
    ...
    async def attempt_recovery(
        ...
        try:
            ...
            # Strategy 2: Use template-based responses
                ...
                app_logger.warning(f"LangGraph recovery attempt {attempt_number}: template responses")
    ...
    def _get_intent_based_response(self, user_message: str) -> str:
        ...
        # Pricing intent
        if any(word in user_message for word in ["preço", "valor", "custo", "mensalidade", "quanto"]):
            return (
        ...
        # Scheduling intent
        elif any(word in user_message for word in ["agendar", "marcar", "visita", "entrevista", "horário"]):
            return (
        ...
        # Information intent
        elif any(word in user_message for word in ["kumon", "programa", "matemática", "português", "inglês"]):
            return (
...
class PostprocessingRecoveryHandler(StageRecoveryHandler):
    ...
    async def attempt_recovery(
        ...
        try:
            ...
            if attempt_number <= 2:
                app_logger.info(f"Postprocessing recovery attempt {attempt_number}: simplified formatting")
            ...
            # Strategy 2: Bypass formatting entirely
                ...
                app_logger.warning(f"Postprocessing recovery attempt {attempt_number}: bypass formatting")
    ...
    async def attempt_recovery(
        ...
        try:
            ...
            if attempt_number <= 3 and "timeout" not in error_message:
                ...
                app_logger.info(f"Delivery recovery attempt {attempt_number}: retry after {wait_time}s")
            ...
            # Strategy 2: Try alternative delivery method (if available)
                ...
                app_logger.warning(f"Delivery recovery attempt {attempt_number}: alternative delivery")
...
class PipelineRecoverySystem:
    ...
    def __init__(self):
        ...
        app_logger.info("Pipeline Recovery System initialized successfully", extra={
    ...
    async def attempt_stage_recovery(
        ...
        try:
            app_logger.info(f"Starting recovery for {failed_stage.value} in execution {execution_id}", extra={
                "error_type": type(error).__name__,
            ...
            for attempt_number in range(1, max_attempts + 1):
                ...
                try:
                    app_logger.info(f"Recovery attempt {attempt_number}/{max_attempts} for {failed_stage.value}")
                    
                    # Attempt recovery
                    recovery_result, fallback_response, recovered_state = await recovery_handler.attempt_recovery(
                        error, context, attempt_number
                    )
                    ...
                    if recovery_result in [RecoveryResult.SUCCESS, RecoveryResult.PARTIAL]:
                        app_logger.info(f"Recovery successful after {attempt_number} attempts", extra={
        ...
        try:
            ...
            if stage_name not in self.recovery_metrics.stage_recovery_stats:
                self.recovery_metrics.stage_recovery_stats[stage_name] = {
                    "total": 0, "success": 0, "partial": 0, "failed": 0, "escalated": 0
                }
            ...
            self.recovery_metrics.recovery_success_rate = (
                (successful_attempts / self.recovery_metrics.total_recovery_attempts) * 100
            )
    ...
    async def _check_auto_escalation(self, failed_stage: PipelineStage, phone_number: str):
        ...
        try:
            ...
            recent_failures = [
                attempt for attempt in self.recovery_attempts[-20:]  # Last 20 attempts
                if (
                    attempt.failed_stage == failed_stage and
                    attempt.result == RecoveryResult.FAILED and
                    (datetime.now() - attempt.timestamp).total_seconds() < 3600  # Last hour
                )
            ]
    ...
    async def _trigger_escalation(self, failed_stage: PipelineStage, reason: str, phone_number: str):
        ...
        try:
            ...
            app_logger.critical(f"Pipeline escalation triggered: {reason}", extra={
