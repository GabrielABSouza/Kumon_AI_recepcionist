...
class PromptInjectionDefense:
    ...
    async def detect_injection(
        ...
        return {
            ...
            "detection_layers": {
                "pattern_based": bool(isinstance(pattern_result, dict) and pattern_result["confidence"] > 0.5),
                "behavioral": bool(isinstance(behavioral_result, dict) and behavioral_result["confidence"] > 0.5),
                "ml_based": bool(isinstance(ml_result, dict) and ml_result["confidence"] > 0.5),
                "context": bool(isinstance(context_result, dict) and context_result["confidence"] > 0.5)
            }
        }
    ...
    async def _behavioral_detection(
        ...
        behavior_entry = {
            ...
            "contains_instructions": bool(re.search(r'ignore|forget|new task|system', user_input, re.IGNORECASE)),
            "unusual_formatting": bool(re.search(r'\\n|\\t|\\r|---|\*\*\*', user_input)),
            "suspicious_keywords": len(re.findall(r'prompt|instruction|system|admin|root|bypass', user_input, re.IGNORECASE))
        }
    ...
    def _extract_ml_features(self, user_input: str) -> Dict[str, int]:
        ...
        return {
            "unusual_keywords": len(re.findall(
                r'ignore|forget|bypass|override|jailbreak|prompt|instruction|system', 
                input_lower
            )),
            "instruction_patterns": len(re.findall(
                r'(?:now|from now on|instead|actually|really|truly)\s+(?:you|your|do|are|will)', 
                input_lower
            )),
            "role_switches": len(re.findall(
                r'(?:you are|act as|pretend|imagine|roleplay|play the role)', 
                input_lower
            )),
            "system_references": len(re.findall(
                r'system|admin|root|developer|programmer|creator|anthropic', 
                input_lower
            )),
            "escape_sequences": len(re.findall(r'\\n|\\t|\\r|```|---|===|\*\*\*', user_input)),
            "prompt_delimiters": len(re.findall(r'###|"""|\[INST\]|\[/INST\]|<\|.*\|>', user_input))
        }
    ...
    async def _context_analysis(
        ...
        if re.search(r'what (?:are|is) your (?:instruction|prompt|system|rule)', user_input, re.IGNORECASE):
            confidence += 0.7
            severity += 0.8
        ...
        if re.search(r'can you|are you (?:able|allowed) to', user_input, re.IGNORECASE):
            boundary_tests = len(re.findall(r'can you|are you (?:able|allowed) to', user_input, re.IGNORECASE))
            if boundary_tests > 1:
                confidence += 0.3
                severity += 0.4
