...
def _get_continuation_decision(state: Dict[str, Any]) -> str | None:
    ...
    QUALIFICATION_REQUIRED_VARS = [
        "parent_name", "beneficiary_type", "student_name", "student_age", "program_interests"
    ]
    collected_data = state.get("collected_data", {})
    if collected_data.get("parent_name"): # Um bom indicador de que a qualificação começou
        missing_vars = [var for var in QUALIFICATION_REQUIRED_VARS if not collected_data.get(var)]
        if missing_vars:
            logger.info(f"ROUTER|Rule|Applying qualification continuation. Missing {len(missing_vars)} vars.")
            return "qualification_node"
...
async def master_router(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"MASTER_ROUTER|NLU Result|intent={primary_intent}|entities={nlu_result.get('entities')}")
        ...
        if primary_intent in interrupt_intents:
            ...
            logger.info(f"MASTER_ROUTER|Decision|Priority 1: User Interruption -> {final_decision}")
        else:
            ...
            if continuation_node:
                ...
                logger.info(f"MASTER_ROUTER|Decision|Priority 2: Business Continuation -> {final_decision}")
            else:
                # Prioridade 3: Novo fluxo baseado na IA
                final_decision = _map_intent_to_node(primary_intent)
                logger.info(f"MASTER_ROUTER|Decision|Priority 3: New Flow -> {final_decision}")
