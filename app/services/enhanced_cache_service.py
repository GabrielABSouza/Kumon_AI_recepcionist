...
class CacheLayer(Enum):
    ...
    L1 = "l1_memory"
    L1_MEMORY = "l1_memory"  # Alias for backward compatibility
    L2 = "l2_redis_session"
    L2_REDIS_SESSION = "l2_redis_session"  # Alias for backward compatibility
    L3 = "l3_redis_rag"
    L3_REDIS_RAG = "l3_redis_rag"  # Alias for backward compatibility
...
class EnhancedCacheService:
        ...
        try:
            ...
            if l1_key in self.l1_cache:
                ...
                if not self._is_expired(entry):
                    ...
                    app_logger.debug(
                        f"L1 cache hit: {key}",
                        extra={"cache_layer": "L1", "access_count": entry.access_count},
                    )
            ...
            app_logger.debug(
                f"Cache miss: {key}", extra={"category": category, "lookup_time_ms": elapsed_ms}
            )
    ...
    async def set(
        self, key: str, value: Any, category: str = "default", ttl: Optional[int] = None
    ) -> bool:
        """
        ...
        try:
            ...
            app_logger.debug(
                f"Cache set: {key}", extra={"category": category, "value_size": len(str(value))}
            )
    ...
    async def _evict_l1_entries(self):
        ...
        try:
            ...
            sorted_entries = sorted(
                self.l1_cache.items(), key=lambda x: x[1].last_accessed or x[1].created_at
            )
    ...
    async def _warm_cache(self):
        ...
        try:
            ...
            greeting_patterns = [
                (
                    "ola",
                    "Olá! Sou Cecília, assistente virtual do Kumon Vila A. Como posso ajudá-lo hoje? 😊",
                ),
            ...
            info_patterns = [
                (
                    "horarios",
                    "Nossos horários de funcionamento são de segunda a sexta, das 8h às 18h.",
                ),
                (
                    "endereco",
                    "Estamos localizados na Rua Amoreira, 571. Salas 6 e 7. Jardim das Laranjeiras.",
                ),
                ("telefone", "Nosso telefone para contato é (51) 99692-1999."),
                (
                    "metodo_kumon",
                    "O método Kumon desenvolve a capacidade de aprendizado autodidático através de material didático programado.",
                ),
                (
                    "disciplinas",
                    "Oferecemos Kumon de Matemática, Português e Inglês para todas as idades.",
                ),
            ]
            ...
            for pattern, response in greeting_patterns + info_patterns:
                await self.set(
                    key=self._generate_cache_key(pattern), value=response, category="rag"
                )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            "performance_metrics": {
                ...
                "error_rate_percentage": (self.metrics.errors / max(1, self.metrics.total_requests))
                * 100,
            },
            "layer_metrics": {
            ...
            "target_metrics": {
                ...
                "performance_assessment": (
                    ...
                    else "GOOD" if self.metrics.hit_rate >= 80 else "NEEDS_IMPROVEMENT"
                ),
            },
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "redis_connections": {"sessions_pool": "connected", "rag_pool": "connected"},
            }
